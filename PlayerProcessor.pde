public class PlayerProcessor {

  private OpenCV opencv;
  private ArrayList<Contour> contours;
  private HueRange hue;

  public PlayerProcessor(OpenCV opencv, HueRange hue) {

    this.opencv = opencv;
    this.contours =  new ArrayList<Contour>();;
    this.hue = hue;
  }


  public OpenCV getOpencv() {
    return opencv;
  }

  public void setOpencv(OpenCV opencv) {
    this.opencv = opencv;
  }

  public ArrayList<Contour> getContours() {
    return contours;
  }

  public void setContours(ArrayList<Contour> contours) {
    this.contours = contours;
  }

  public HueRange getHue() {
    return hue;
  }

  public void setHue(HueRange hue) {
    this.hue = hue;
  }
  
  public void updatePlayer(PImage video, Player player) {
    OpenCV opencv = this.getOpencv();
    ArrayList<Contour> contours = this.getContours();
    HueRange hue = this.getHue();
    
    Rectangle r = new Rectangle();

    // <2> Load the new frame of our movie in to OpenCV
    opencv.loadImage(video);

    // Tell OpenCV to use color information
    opencv.useColor();
    src = opencv.getSnapshot();

    // <3> Tell OpenCV to work in HSV color space.
    opencv.useColor(HSB);

    // <4> Copy the Hue channel of our image into 
    //     the gray channel, which we process.
    opencv.setGray(opencv.getH().clone());

    // <5> Filter the image based on the range of 
    //     hue values that match the object we want to track.
    opencv.inRange(hue.getLowerLimit(), hue.getHigherLimit());

    // <6> Get the processed image for reference.
    colorFilteredImage = opencv.getSnapshot();

    ///////////////////////////////////////////
    // We could process our image here!
    // See ImageFiltering.pde
    ///////////////////////////////////////////

    // <7> Find contours in our range image.
    //     Passing 'true' sorts them by descending area.
    contours = opencv.findContours(true, true);

    // <8> Display background images
    //image(src, 0, 0);
    colorFilteredImage.resize(colorFilteredImage.width*2, colorFilteredImage.height*2);
    image(colorFilteredImage, width/2, 0);
    colorFilteredImage.resize(colorFilteredImage.width/2, colorFilteredImage.height/2);
    // <9> Check to make sure we've found any contours
    if (contours.size() > 0) {
      // <9> Get the first contour, which will be the largest one
      Contour biggestContour = contours.get(0);


      // <10> Find the bounding box of the largest contour,
      //      and hence our object.
      r = biggestContour.getBoundingBox();
    }

    player.setRectangle(r);
  }
}